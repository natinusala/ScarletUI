...
public extension _LayoutTargetNode {
    ...
    func layoutIfNeeded() {
        if YGNodeIsDirty(self.ygNode) {
            layoutLogger.debug("\(self) is dirty, calculating layout")
    ...
    private func calculateLayout() {
        if let parent = self.layoutParent {
            ...
            layoutLogger.debug("Calling `YGNodeCalculateLayout` on \(self)")
    ...
    private func updateLayout(parentX: Float, parentY: Float) {
        ...
        layoutLogger.debug("New layout of \(self): \(self.layout)")
